...
def objective(trial, data):
    ...
    # Hyperparameters
    latent_dim = trial.suggest_categorical('latent_dim', [16, 24, 32, 48, 64])
    valid_hidden = [h for h in [32, 64, 96, 128] if h > latent_dim]
    hidden_dim = trial.suggest_categorical('hidden_dim', valid_hidden)
    ...
    # Constraints: Hidden > Latent AND Hidden > Fusion
    # if hidden_dim <= latent_dim or hidden_dim <= fusion_dim:
...
if __name__ == "__main__":
    ...
    study.optimize(lambda trial: objective(trial, data), n_trials=100, show_progress_bar=True)
    ...
    df.to_csv("best_hybrid_params_knn.csv", index=False)
    print("Saved best params to 'best_hybrid_params_knn.csv'")
