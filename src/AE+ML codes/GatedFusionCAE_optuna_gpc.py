...
def objective(trial, data):
    ...
    fusion_dim = trial.suggest_categorical('fusion_dim', [32, 64, 96])
    ...
    for fold, (t_idx, v_idx) in enumerate(kf.split(rna, Y)):
        ...
        # Eval Gaussian Process Classifier
        enc_r.eval(); enc_m.eval(); enc_c.eval(); fusion.eval()
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    df.to_csv("best_hybrid_params_gpc.csv", index=False)
    print("Saved best params to 'best_hybrid_params_gpc.csv'")
