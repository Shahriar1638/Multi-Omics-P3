...
if HAS_SHAP and 'rna_df' in locals():
    ...
    def prepare_full_data_for_shap(df_rna, df_meth, df_cnv, Y):
        ...
        # 1. Drop all-NaN columns first (Clean Alignment)
        # We do this manually to ensure we drop the SAME names as columns
            mask_valid = ~np.isnan(X).all(axis=0)
            
            if not np.all(mask_valid):
                dropped_count = np.sum(~mask_valid)
                print(f"   [{omic_name}] Pre-dropping {dropped_count} all-NaN cols.")
                X = X[:, mask_valid]
                # Filter names list using the boolean mask
                names = [n for i, n in enumerate(names) if mask_valid[i]]
            
            # 2. KNN Imputation (Prevent shape change)
            # keep_empty_features=True is ideal (sklearn 1.2+), but we add a fallback just in case
            try:
                imp = KNNImputer(n_neighbors=12, keep_empty_features=True)
                X_imputed = imp.fit_transform(X)
            except TypeError:
                # Fallback for older sklearn that doesn't support keep_empty_features
                imp = KNNImputer(n_neighbors=12)
                X_imputed = imp.fit_transform(X)

            # 3. CRITICAL FIX: Handle Shape Mismatch
            # If Imputer dropped columns (X_imputed is smaller), it means some cols were not imputable.
            # We must recover the original shape to keep names aligned, filling missing with 0.
            if X_imputed.shape[1] < X.shape[1]:
                print(f"   [{omic_name}] Imputer dropped columns. Restoring shape with 0-fill...")
                # This is complex to restore perfectly, so it is better to FILL NaNs *before* # or force fill remaining NaNs *instead* of dropping.
                
                # RE-STRATEGY: Fill NaNs that KNN missed with 0 instead of dropping
                # We use the original X, replace the parts KNN found, and 0-fill the rest.
                # (Simplest approach: Just fill remaining NaNs in X_imputed if it kept shape, 
                # or if it dropped, we should have used a SimpleImputer backup).
                
                # Simplest robust fix for your pipeline:
                # Run SimpleImputer on the ORIGINAL X for columns that KNN failed on.
                from sklearn.impute import SimpleImputer
                simp = SimpleImputer(strategy='constant', fill_value=0)
                X_imputed = simp.fit_transform(X) # This never drops columns
                
            # If shape is preserved but NaNs remain (which might cause downstream issues):
            if np.isnan(X_imputed).any():
                X_imputed = np.nan_to_num(X_imputed, nan=0.0)

            # 4. Final Verification
            if X_imputed.shape[1] != len(names):
                print(f"   [{omic_name}] CRITICAL WARNING: Alignment lost. X: {X_imputed.shape[1]}, Names: {len(names)}")
                # Pad with explicit error names if we still have a mismatch
                if X_imputed.shape[1] > len(names):
                    diff = X_imputed.shape[1] - len(names)
                    names += [f"LOST_NAME_{i}" for i in range(diff)]
                else:
                    names = names[:X_imputed.shape[1]]

            print(f"   [{omic_name}] Impute Output: {X_imputed.shape}, names: {len(names)}")
            return X_imputed, names
        # Variance Filter
        def var_filter_with_names(X, names, top_k):
            ...
            new_X = X[:, top_idx]
            new_names = [names[i] for i in top_idx]
            print(f"   [SHAP Prep] VarFilter Output: {new_X.shape}, names: {len(new_names)}")
            return new_X, new_names

        # --- TRACKING LOGIC START ---
        feature_tracker = {
            'RNA': {'initial': [], 'after_var': [], 'after_impute': [], 'after_mrmr': []},
            'Meth': {'initial': [], 'after_var': [], 'after_impute': [], 'after_mrmr': []},
            'CNV': {'initial': [], 'after_var': [], 'after_impute': [], 'after_mrmr': []}
        }
        
        feature_tracker['RNA']['initial'] = [str(n) for n in rna_cols]
        feature_tracker['Meth']['initial'] = [str(n) for n in meth_cols]
        feature_tracker['CNV']['initial'] = [str(n) for n in cnv_cols]
        # --- TRACKING LOGIC END ---

        print("   [SHAP Prep] Variance Filtering...")
        X_r, rna_cols = var_filter_with_names(X_r, rna_cols, 5000)
        feature_tracker['RNA']['after_var'] = [str(n) for n in rna_cols]

        X_m, meth_cols = var_filter_with_names(X_m, meth_cols, 10000)
        feature_tracker['Meth']['after_var'] = [str(n) for n in meth_cols]

        X_c, cnv_cols = var_filter_with_names(X_c, cnv_cols, 8000)
        feature_tracker['CNV']['after_var'] = [str(n) for n in cnv_cols]

        # KNN Imputation with robust name tracking
        print("   [SHAP Prep] Imputing...")
        X_r, rna_cols = safe_impute(X_r, rna_cols, "RNA")
        feature_tracker['RNA']['after_impute'] = [str(n) for n in rna_cols]

        X_m, meth_cols = safe_impute(X_m, meth_cols, "Meth")
        feature_tracker['Meth']['after_impute'] = [str(n) for n in meth_cols]

        X_c, cnv_cols = safe_impute(X_c, cnv_cols, "CNV")
        feature_tracker['CNV']['after_impute'] = [str(n) for n in cnv_cols]
        ...
        def mrmr_with_names(X, y, names, top_k, omic_tag="Data"):
            ...
            print(f"   [{omic_tag}] mRMR Output: {X[:, valid_indices].shape}, names: {len([names[i] for i in valid_indices])}")
        ...
        feature_tracker['RNA']['after_mrmr'] = [str(n) for n in rna_cols]

        X_m, meth_cols = mrmr_with_names(X_m, Y, meth_cols, 400, "Meth")
        feature_tracker['Meth']['after_mrmr'] = [str(n) for n in meth_cols]

        X_c, cnv_cols = mrmr_with_names(X_c, Y, cnv_cols, 300, "CNV")
        feature_tracker['CNV']['after_mrmr'] = [str(n) for n in cnv_cols]
        
        print(f"   [SHAP Prep] Final Shapes: RNA {X_r.shape}, Meth {X_m.shape}, CNV {X_c.shape}")

        # Save Tracker
        try:
             with open('feature_tracking_debug.json', 'w') as f:
                 json.dump(feature_tracker, f, indent=4)
             print("   [DEBUG] Feature tracking saved to feature_tracking_debug.json")
        except Exception as e:
             print(f"   [DEBUG] Failed to save feature tracking: {e}")
    ...
    print(f"DEBUG: Returned from prep. X_r: {X_r_full.shape}, names: {len(rna_names)}")
    ...
    print("   Training for SHAP (1 epochs)...")
